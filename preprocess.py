...
def get_raw_files(raw_dir, sources):
    ...
    for d in sources:
        src_file, trg_file = download_and_extract(raw_dir, d["url"], d["src"],
                                                  d["trg"])
...
def encode_files(bpe, src_in_file, trg_in_file, data_dir, prefix):
    ...
    if os.path.isfile(src_out_file) and os.path.isfile(trg_out_file):
        sys.stderr.write(
            "Encoded files found, skip the encoding process ...\n")
...
def main():
    ...
    field = torchtext.data.Field(tokenize=str.split,
                                 lower=True,
...
def main_wo_bpe():
    ...
    SRC = torchtext.data.Field(tokenize=tokenize_src,
                               lower=not opt.keep_case,
                               pad_token=Constants.PAD_WORD,
                               init_token=Constants.BOS_WORD,
                               eos_token=Constants.EOS_WORD)

    TRG = torchtext.data.Field(tokenize=tokenize_trg,
                               lower=not opt.keep_case,
                               pad_token=Constants.PAD_WORD,
                               init_token=Constants.BOS_WORD,
                               eos_token=Constants.EOS_WORD)
